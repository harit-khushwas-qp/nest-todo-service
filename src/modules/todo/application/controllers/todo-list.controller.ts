...
export class TodoListController {
  ...
  @Get()
  @ApiOperation({ summary: 'Get all todo lists', description: 'Retrieve all todo lists for the authenticated user' })
  @ApiResponse({ status: 200, description: 'List of todo lists retrieved successfully' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  async getLists(@Req() request: any) {
    return this.todoService.getListsForUser(request.user.userId)
  }

  @Get(':id')
  @ApiOperation({ summary: 'Get todo list by ID', description: 'Retrieve a specific todo list by its ID' })
  ...
  @Post()
  @ApiOperation({ summary: 'Create todo list', description: 'Create a new todo list' })
  ...
  @Post(':id/todos')
  @ApiOperation({ summary: 'Add todo to list', description: 'Add an existing todo to a todo list' })
  @ApiParam({ name: 'id', type: Number, description: 'Todo List ID' })
  ...
  async addTodoToList(
    ...
    return this.todoService.addTodoToList(request.user.userId, listId, body.todoId)
  }

  @Patch('order')
  @ApiOperation({ summary: 'Reorder todo lists', description: 'Change the order of todo lists' })
